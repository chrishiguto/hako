//! The report contract — the only structured channel from agent to
//! engine, written to end every invocation. The status semantics, the
//! questions a paused run asks, and the answers a human sends back
//! are uniform across kernels, so HITL behaves the same whatever loop
//! is running. The pipeline kernel's stage report shapes build on
//! that vocabulary: one strict type per stage, mirroring the flow
//! format's pattern — Rust types as the source of truth, generated
//! JSON Schemas (committed under `schemas/report/`, drift-checked in
//! CI) for the preamble to quote, and a repair re-prompt fed by the
//! strict parse's errors.
//!
...
/// One stage's report, whichever stage wrote it — what a kernel holds
/// when it carries reports across stages. Parsing dispatches on the
/// stage the kernel just ran: the report file is not self-describing,
/// the pipeline's position is what names it.
...
/// cannot disagree. Committed under `schemas/report/`, drift-checked
/// in CI, and quoted verbatim in the stage's preamble.
#[cfg(feature = "schema")]
